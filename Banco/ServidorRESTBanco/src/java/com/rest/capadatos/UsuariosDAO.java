...
public class UsuariosDAO implements Metodos {
    ...
    @Override
    public List<Usuarios> login(String userName, int nip) {
        List<Usuarios> datos = new ArrayList<Usuarios>();
        int activo = 0;
        String estado = "SELECT estado FROM usuarios WHERE username = ? and nip = ?";
        try {
        ...
        if (activo == 1) {
            String sql = "select * from usuarios where username = ? and nip = ?";
            try {
                conn = conex.getConnection();
